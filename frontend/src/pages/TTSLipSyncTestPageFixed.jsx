/**
 * TTS + Lip Sync Test Page - FIXED VERSION
 * Replaces random mouth movement with proper Conv-AI viseme-based phoneme synchronization
 * Uses proven viseme mapping system for accurate lip sync
 */
...
import { synthesizeSpeech, textToPhonemes, phonemesToVisemes } from '../utils/speechProcessing';
...
// Enhanced Avatar with Conv-AI Viseme-based Lip Sync
function SavannahAvatar({ isPlaying, currentViseme, ...props }) {
  ...
  // Frame synchronization with Conv-AI pattern
  useFrame((state) => {
...
// Enhanced Status Display with Viseme Information
function StatusDisplay({ isPlaying, currentViseme, currentPhoneme }) {
  return (
      ...
      <div className="animation-status">
        ...
        {currentPhoneme && (
          <div className="phoneme-info">
            <small>Phoneme: {currentPhoneme}</small>
          </div>
        )}
        {currentViseme && Object.keys(currentViseme).length > 0 && (
          <div className="viseme-info">
            <small>Viseme: {Object.keys(currentViseme).join(', ')}</small>
          </div>
        )}
      </div>
    </div>
...
export default function TTSLipSyncTestPage() {
  ...
  const [currentViseme, setCurrentViseme] = useState({ 0: 1.0 }); // Start with silence
  const [currentPhoneme, setCurrentPhoneme] = useState(null);

  // Enhanced speech synthesis with proper viseme synchronization
  const handleSpeak = useCallback(async () => {
    ...
    try {
      ...
      // Use the enhanced speech synthesis with viseme callback
      await synthesizeSpeech(
        ...
        (visemeData) => {
          // Update current viseme for the avatar
          setCurrentViseme(visemeData);
          
          // Extract phoneme info for display
          if (visemeData && Object.keys(visemeData).length > 0) {
            const visemeKey = Object.keys(visemeData)[0];
            setCurrentPhoneme(visemeKey);
          }
        }
      );
  ...
  return (
    <div className="tts-lipsync-test-page">
      <div className="page-header">
        <h1>TTS + Lip Sync Test (Conv-AI Viseme System)</h1>
        <p>Fixed implementation using proper phoneme-to-viseme mapping for accurate lip synchronization</p>
      </div>
          ...
          <div className="avatar-overlay">
            <div className="status-info">
              <span className={`avatar-status ${isPlaying ? 'speaking' : 'idle'}`}>
                Avatar: {isPlaying ? 'Speaking with Visemes' : 'Idle'}
              </span>
            </div>
        ...
        <div className="controls-container">
          ...
          <div className="control-section">
            ...
            <div className="system-stats">
              ...
              <div className="stat-item">
                <span className="stat-label">Viseme System:</span>
                <span className="stat-value">✅ Conv-AI Mapping</span>
              </div>
              <div className="stat-item">
              ...
              <div className="stat-item">
                <span className="stat-label">Lip Sync Mode:</span>
                <span className="stat-value">✅ Phoneme-Based</span>
              </div>
            </div>
          </div>
        </div>
